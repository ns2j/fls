...
namespace LazyPCSCFelicaLite
{
	...
	{
	public:
		uint8_t current_SK1[8];
		uint8_t current_SK2[8];
		uint8_t current_RC1[8];
	...
	public:
		...
		bool autoConnectToFelica(uint32_t timeout=0)
		{
			...
			{
				if( e.errorcode() == PCSCFatalException::CARD_HAS_LOCKED_BY_ANOTHER_PROGRAM )
				{
#ifdef _WIN32
					Sleep(100);
#else
					struct timespec ts = {.tv_sec = 0, .tv_nsec = 100 * 1000};
					nanosleep(&ts, &ts); //解放待ち時間
#endif
					try {
						connectCard();
					} catch (...) {
		...
		void openService()
		{
			...
			res = SCardListReaders(hContext, NULL, (LPSTR)&lpszReaderName, &dwAutoAllocate);
			if ( res != SCARD_S_SUCCESS )
			{
		...
		bool isCardSet(uint32_t state = SCARD_STATE_UNAWARE, uint32_t timeout = 0)
		{
			...
			//			LONG res = SCardGetStatusChangeA(hContext, (DWORD)timeout, &readerState, 1);
				LONG res = SCardGetStatusChange(hContext, (DWORD)timeout, &readerState, 1);
...
*/
		...
		void connectCard(bool exclusive = true)
		{
			...
			LONG res = SCardConnect(hContext, activeReaderName.c_str(), dwShareMode, SCARD_PROTOCOL_T0 | SCARD_PROTOCOL_T1, &hCard, &dwActiveProtocol);
			if ( res != SCARD_S_SUCCESS )
			{
			...
			{
				cardSystemcode = readSystemcode();
				if ( cardSystemcode == 0x88B4 ) {
					// デフォルトで 0x0009 (Read/Write) を選択
					selectService(0x0009);
				}
			}
			debugInfo2(__func__, " カードに接続しました(カード種別:%02X, システムコード:%04X)\n", cardTypecode, cardSystemcode);
		}

		void selectService(uint16_t serviceCode)
		{
			BYTE pbSendBuffer[7] = {0xFF, 0xA4, 0x00, 0x01, 0x02, (BYTE)(serviceCode & 0xFF), (BYTE)((serviceCode >> 8) & 0xFF)};
			BYTE pbRecvBuffer[256];
			DWORD pcbRecvLength = 256;
			SCardTransmit(hCard, SCARD_PCI_T1, pbSendBuffer, 7, NULL, pbRecvBuffer, &pcbRecvLength);
		}
		...
		void connectDirect()
		{
			...
			LONG res = SCardConnect(hContext, activeReaderName.c_str(), SCARD_SHARE_DIRECT, 0, &hCard, &dwActiveProtocol);
			if ( res != SCARD_S_SUCCESS )
			{
		...
		void updateBinaryWithMAC_A_Session(uint16_t adr, uint8_t dat[16]) {
			uint8_t mac[16];
			uint8_t wcnt[16];
			readBinary(ADDRESS_WCNT, wcnt);
			makeWriteMAC_A(adr, wcnt, dat, current_RC1, current_SK1, current_SK2, mac);
			mac[8] = wcnt[0]; mac[9] = wcnt[1]; mac[10] = wcnt[2]; mac[11] = wcnt[3];
			updateBinaryWithMAC_A(adr, dat, mac);
		}
		void writeCardKeyWithMAC_A(uint8_t ck1[8], uint8_t ck2[8], uint8_t nck1[8], uint8_t nck2[8]) {
			uint8_t dat[16]; for(int i=0; i<8; i++) { dat[7-i]=nck1[i]; dat[15-i]=nck2[i]; }
			updateBinaryWithMAC_A_Auto(ADDRESS_CK, dat, ck1, ck2);
		}
		void readNdefURIWithMAC_A(std::string& uri) {
			uint8_t buf[16], mac[16];
			readBinaryWithMAC_A(ADDRESS_SPAD1, buf, mac);
			int len = buf[14] - 1;
			if (len <= 0) return;

			int blocks = (len + 7 + 15) / 16;
			...
			for (int i = 0; i < blocks; i++) {
				uint8_t spad_buf[16];
				readBinaryWithMAC_A(ADDRESS_SPAD2 + i, spad_buf, mac);
				for (int k = 0; k < 16; k++) {
					if (i == 0 && k == 0) {
						mode = spad_buf[0];
						continue;
					...
					if (chars_read < len) {
						result += (char)spad_buf[k];
						chars_read++;
					}
				}
			}

			if (mode == 0x01) uri = "http://www." + result;
			else if (mode == 0x02) uri = "https://www." + result;
			else if (mode == 0x03) uri = "http://" + result;
			else if (mode == 0x04) uri = "https://" + result;
			else if (mode == 0x05) uri = "tel:" + result;
			else if (mode == 0x06) uri = "mailto:" + result;
			else uri = result;
		}

		void readNdefURI(std::string& uri) {
			uint8_t buf[16];
			readBinary(ADDRESS_SPAD1, buf);
			int len = buf[14] - 1;
			if (len <= 0) return;
			...
			for (int i = 0; i < blocks; i++) {
				uint8_t spad_buf[16];
				readBinary(ADDRESS_SPAD2 + i, spad_buf);
				for (int k = 0; k < 16; k++) {
					if (i == 0 && k == 0) {
						mode = spad_buf[0];
						continue;
		...
		void readNdefContent(std::string& content) {
			uint8_t buf0[16];
			readBinary(ADDRESS_SPAD0, buf0);
			int Ln = buf0[13];
			
			if (Ln > 0 && Ln <= 192) {
				uint8_t ndef_buf[256] = {0};
				int blocks = (Ln + 15) / 16;
				if (blocks > 12) blocks = 12;

				for (int i = 0; i < blocks; i++) {
					uint8_t buf[16];
					readBinary(ADDRESS_SPAD1 + i, buf);
					for (int k = 0; k < 16; k++) ndef_buf[i * 16 + k] = buf[k];
				}

				if (ndef_buf[0] == 0xD1 && ndef_buf[3] == 0x54) { // 'T' (Text)
					int payload_len = ndef_buf[2];
					if (payload_len > 250) return; // バッファオーバーラン防止
					int lang_len = ndef_buf[4] & 0x3F;
					int text_len = payload_len - 1 - lang_len;
					if (text_len > 0) {
						content.assign((char*)&ndef_buf[5 + lang_len], text_len);
						return;
					}
				} else if (ndef_buf[0] == 0xD1 && ndef_buf[3] == 0x55) { // 'U' (URI)
					int payload_len = ndef_buf[2];
					if (payload_len > 250) return; // バッファオーバーラン防止
					if (payload_len > 0) {
						uint8_t mode = ndef_buf[4];
						int uri_len = payload_len - 1;
						std::string result((char*)&ndef_buf[5], uri_len);
						std::string uri = "";
						if (mode == 0x01) uri = "http://www." + result;
						else if (mode == 0x02) uri = "https://www." + result;
						else if (mode == 0x03) uri = "http://" + result;
		...
		void readBinary(uint16_t adr, uint8_t dat[16])
		{
			...
			//コマンド送信 (システムブロック 0x0080〜 は RO サービスコード 0x000B を一時的に選択)
			bool needRO = (cardSystemcode == 0x88B4 && adr >= 0x0080);
			if (needRO) selectService(0x000B);
			LONG res = SCardTransmit(hCard, SCARD_PCI_T1, pbSendBuffer, bufsize, NULL, pbRecvBuffer, &pcbRecvLength);
			if (needRO) selectService(0x0009);
			if ( res == SCARD_W_REMOVED_CARD )
			{
		...
		void readBinaryWithMAC(uint16_t adr, uint8_t dat[16], uint8_t mac[16])
		{
			...
			//コマンド送信 (MAC付き読み出し時は RO サービスコード 0x000B を一時的に選択)
			if (cardSystemcode == 0x88B4) selectService(0x000B);
			LONG res = SCardTransmit(hCard, SCARD_PCI_T1, pbSendBuffer, bufsize, NULL, pbRecvBuffer, &pcbRecvLength);
			if (cardSystemcode == 0x88B4) selectService(0x0009);
			if ( res == SCARD_W_REMOVED_CARD )
			{
				debugInfo2(__func__, " カードが取り外されました．\n");
				throw PCSCCardRemovedException("カードが取り外されました．");
		...
		void updateBinary(uint16_t adr, uint8_t dat[16])
		{
			...
			if ( SW1 != 0x90 || SW2 != 0x00 )
			{
				if ( SW1 == 0x63 && SW2 == 0x8A )
				{
					debugInfo2(__func__, " アドレス:%04X メモリ寿命警告(63 8A)を受信しましたが、書き込みは成功しているため続行します。\n", adr);
				}
				else
				{
					debugInfo2(__func__, " アドレス:%04X カードはエラー応答しました．SW1=%02X,SW2=%02X\n", adr, SW1, SW2);
		...
		*/
		void readCardTypeString(char *cardTypeString = NULL)
		{
			...
			LONG res = SCardTransmit(hCard, SCARD_PCI_T1, pbSendBuffer, sizeof(pbSendBuffer), NULL, pbRecvBuffer, &pcbRecvLength);
			if ( res != SCARD_S_SUCCESS ) {
				throw PCSCCommandException("Command error", res);
			}
			...
			if ( SW1 != 0x90 || SW2 != 0x00 ) {
				throw PCSCErrorException("Card error", SW1, SW2);
			}
...
#define SCARD_CTL_CODE(code) (0x42000000 + (code))
#endif
			LONG res = SCardControl(hCard, SCARD_CTL_CODE(1), lpInBuffer, sizeof(lpInBuffer), lpOutBuffer, sizeof(lpOutBuffer), &lpBytesReturned);
			if ( res != SCARD_S_SUCCESS )
			{
				debugInfo2(__func__, " コマンドに誤りがあるか不適切な状態です %X\n", res);
				throw PCSCCommandException("コマンドに誤りがあるか不適切な状態", res);
		...
		void FirstIssue()
		{
			...
			//MC_ALL = 00 (RO, 1次発行済フラグ)
			//SYS_OP = 0x01 (NDEF対応, P2P無効)
			//MC_SP_REG_R_RESTR = 00 3C (SPAD10-SPAD13で読み出しMAC必須)
			//MC_SP_REG_W_MAC_A と MC_SP_REG_W_RESTR を 0xFF 0x3F にしてSPAD0-SPAD13全てで書き込みMAC必須化
			uint8_t mc[16] = {0xFF,0xFF,0x00,0x01,0xFF,0x01,0x00,0x3C,0xFF,0x3F,0xFF,0x3F,0x01,0x00,0x00,0x00};
		...
		// Android等でNDEFが反応しない場合（SYS_OPにNFC-DEPビット等が立っている場合）にSYS_OPを0x01に修正する
		void fixAndroidNdef(uint8_t CK1[8], uint8_t CK2[8])
		{
			if ( !isFelicaLite() ) throw FelicaFatalException("Felica Liteではない", FelicaFatalException::CARD_IS_NOT_FELICA_LITE);
			uint8_t mc[16] = {0};
			readBinary(ADDRESS_MC, mc);
			if (mc[3] != 0x01) {
				mc[3] = 0x01;
				updateBinaryWithMAC_A_Auto(ADDRESS_MC, mc, CK1, CK2);
				debugInfo2(__func__, " SYS_OPを0x01に修正しました\n");
			}
		}
		...
		{
			if ( !isFelicaLite() ) throw FelicaFatalException("Felica Liteではない", FelicaFatalException::CARD_IS_NOT_FELICA_LITE);
			if ( str == NULL ) throw std::invalid_argument("NullPointerException");

			int len = 0;
			for ( len = 0; len<184; len++ ) {
				if ( str[len] == '\0' ) break;
			}
			if ( len >= 184 ) throw std::length_error("URIが長すぎる");

			int payload_len = len + 1;
			int ndef_msg_len = 4 + payload_len; // D1, 01, PL, 55, MD, ...

			uint8_t ndef_head[16] = {0x10,0x04,0x01,0x00,0x0C,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
			ndef_head[13] = ndef_msg_len & 0xFF;

			int sum = 0;
			for ( int i = 0; i<14; i++ ) sum += ndef_head[i]; // Checksum is sum of 0 to 13
			ndef_head[14] = (sum >> 8) & 0xFF;
			ndef_head[15] = sum & 0xFF;

			uint8_t ndef_msg[256] = {0};
			ndef_msg[0] = 0xD1; // MB=1, ME=1, SR=1, TNF=1
			ndef_msg[1] = 0x01; // Type Length = 1
			ndef_msg[2] = payload_len & 0xFF; // Payload Length
			ndef_msg[3] = 0x55; // Type 'U'
			ndef_msg[4] = mode;
			for ( int i = 0; i<len; i++ ) ndef_msg[5+i] = str[i];

			updateBinary(ADDRESS_SPAD0, ndef_head);

			int blocks = (ndef_msg_len + 15) / 16;
			if (blocks > 12) blocks = 12;

			for ( int i = 0; i<blocks; i++ ) {
				unsigned char buf[16] = {0};
				for ( int k = 0; k<16; k++ ) buf[k] = ndef_msg[(i * 16) + k];
				updateBinary(ADDRESS_SPAD1 + i, buf);
			}

			debugInfo2(__func__, " NDEF書き込み完了 %s\n",str);
		}

		//簡易的なNDEF(URI)をMAC付きで書き込む
		void writeNdefURIWithMAC(uint8_t mode, const char str[], uint8_t CK1[8], uint8_t CK2[8])
		{
			...
			updateBinaryWithMAC_A_Auto(ADDRESS_SPAD0, ndef_head, CK1, CK2);
			...
			for ( int i = 0; i<blocks; i++ ) {
				unsigned char buf[16] = {0};
				for ( int k = 0; k<16; k++ ) buf[k] = ndef_msg[(i * 16) + k];
				updateBinaryWithMAC_A_Auto(ADDRESS_SPAD1 + i, buf, CK1, CK2);
			}

			debugInfo2(__func__, " NDEF MAC付き書き込み完了 %s\n",str);
		}

		void writeNdefText(const char str[])
		{
			if ( !isFelicaLite() ) throw FelicaFatalException("Felica Liteではない", FelicaFatalException::CARD_IS_NOT_FELICA_LITE);
			if ( str == NULL ) throw std::invalid_argument("NullPointerException");

			int text_len = strlen(str);
			if (text_len > 184) throw std::length_error("Text is too long (max 184 bytes)");
			
			int payload_len = text_len + 3;
			int Ln = 4 + payload_len;

			uint8_t ndef_head[16] = {0x10,0x04,0x01,0x00,0x0C,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
			ndef_head[11] = 0;
			ndef_head[12] = 0;
			ndef_head[13] = Ln & 0xFF;

			int sum = 0;
			for ( int i = 0; i<15; i++ ) sum += ndef_head[i];
			ndef_head[15] = sum & 0xFF;

			uint8_t ndef_buf[256] = {0};
			ndef_buf[0] = 0xD1;
			ndef_buf[1] = 0x01;
			ndef_buf[2] = payload_len & 0xFF;
			ndef_buf[3] = 0x54; // 'T'
			ndef_buf[4] = 0x02; // UTF-8, len=2
			ndef_buf[5] = 0x6A; // 'j'
			ndef_buf[6] = 0x61; // 'a'
			for (int i=0; i<text_len; i++) ndef_buf[7+i] = str[i];
			...
			for ( int i = 0; i<blocks; i++ ) {
				unsigned char buf[16] = {0};
				for ( int k = 0; k<16; k++ ) buf[k] = ndef_buf[(i * 16) + k];
				updateBinary(ADDRESS_SPAD1 + i, buf);
			}
			debugInfo2(__func__, " NDEFテキスト書き込み完了 %s\n",str);
		}

		void writeNdefTextWithMAC_A_Session(const char str[])
		{
			if ( !isFelicaLite() ) throw FelicaFatalException("Felica Liteではない", FelicaFatalException::CARD_IS_NOT_FELICA_LITE);
			if ( str == NULL ) throw std::invalid_argument("NullPointerException");
			...
			updateBinaryWithMAC_A_Session(ADDRESS_SPAD0, ndef_head);
			...
			for ( int i = 0; i<blocks; i++ ) {
				unsigned char buf[16] = {0};
				for ( int k = 0; k<16; k++ ) buf[k] = ndef_buf[(i * 16) + k];
				updateBinaryWithMAC_A_Session(ADDRESS_SPAD1 + i, buf);
			}
			debugInfo2(__func__, " NDEFテキストMAC付き書き込み完了 %s\n",str);
		}
		...
		//8バイト固定長でDES暗号化を行う
		void desEncryptFixedLength(const uint8_t input[8], const uint8_t input_key[8], const uint8_t iv[8], uint8_t output[8])
		{
#ifdef _WIN32
			...
			try
			{
				if ( BCryptGenerateSymmetricKey(hAlgorithm, &hKey, pKeyObject, KeyObjectSize, (PUCHAR)input_key, dataLength, 0) )
				{
					throw PCSCCryptographicException("BCryptGenerateSymmetricKeyがエラーを返しました");
				}

				if ( BCryptSetProperty(hAlgorithm, BCRYPT_CHAINING_MODE, (PUCHAR)BCRYPT_CHAIN_MODE_CBC, sizeof(BCRYPT_CHAIN_MODE_CBC), 0) )
				{
					// Ignore chain mode set error if unsupported, CBC is usually default
				}

				uint8_t iv_copy[8];
				memcpy(iv_copy, iv, 8);

				if ( BCryptEncrypt(hKey, (PUCHAR)input, dataLength, nullptr, (PUCHAR)iv_copy, dataLength, (PUCHAR)output, dataLength, &res, 0) )
				{
			...
			mbedtls_des_setkey_enc(&des_ctx, input_key);

			// ivの値が破壊されるのでコピーしたものを使う。
			uint8_t iv_copy[8];
			memcpy(iv_copy, iv, 8);

			mbedtls_des_crypt_cbc(&des_ctx, MBEDTLS_DES_ENCRYPT, 8, iv_copy, input, output);
		...
		void desDecryptFixedLength(const uint8_t input[8], const uint8_t key[8], const uint8_t IV[8], uint8_t output[8])
		{
			...
			try
			{
				...
				if ( BCryptDecrypt(hKey, (PUCHAR)input, dataLength, nullptr, (PUCHAR)iv_copy, dataLength, (PUCHAR)output, dataLength, &res, 0) )
				{
			...
			mbedtls_des_crypt_cbc(&des_ctx, MBEDTLS_DES_DECRYPT, 8, iv_copy, input, output);
		...
		void makeMAC(uint8_t BLOCK[16], uint8_t RC1[8], uint8_t SK1[8], uint8_t SK2[8], uint8_t MAC[8])
		{
			...
			DualKey2des3(Tinput1, Tinput2, TXorInput, Tkey2, Tkey1, tmp1, tmp2);
		...
		void writeCardKey(uint8_t CK1[8], uint8_t CK2[8])
		{
			...
			// FeliCa Lite-S仕様: 鍵を有効にするには CKV (Card Key Version) に非ゼロを書き込む必要がある
			uint8_t CKV[16] = {0};
			CKV[0] = 0x01; // バージョン1として有効化
			CKV[1] = 0x00;
			updateBinary(ADDRESS_CKV, CKV);

			debugInfo2(__func__, " CKおよびCKVを書き込みました\n");
		}
		...
		void enableSTATE()
		{
			...
			uint8_t mc[16];
			readBinary(ADDRESS_MC, mc);
			// FeliCa Lite-Sでは、SPADのMAC必須化フラグはバイト8〜11です。1が保護(MAC必須)を意味します。
			// 全てのSPAD領域(0〜13)をMAC必須にするため、0xFF, 0x3Fをセットします。
			mc[8] = 0xFF;
			mc[9] = 0x3F;
			mc[10] = 0xFF;
			mc[11] = 0x3F;
			mc[12] = 0x01; // Byte 12 is MC_STATE_W_MAC_A (01でMAC必須化)
			
			// SPAD10-SPAD13で読み取りにもMACを必須化します
			mc[6] = 0x00;
			mc[7] = 0x3C;
			
			updateBinary(ADDRESS_MC, mc);
			debugInfo2(__func__, " STATEおよびユーザー領域(SPAD)のMAC付き書き込みを有効にしました\n");
		}
